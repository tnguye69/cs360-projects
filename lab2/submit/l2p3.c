 ...
 * This program reads information from the file converted and
   ...
   This program strictly uses system calls, except you read in
   all of the bytes at once. (through hardcoding)
 ***************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
...
main(int argc, char **argv)
{
	...
	if(file == -1) { perror(argv[1]); exit(1); }

	/* read the file into buffer */
	unsigned char buffer[350000];   // hardcode buffer into 350000 bytes
	length = read(file, buffer, 350000);
	buffer[length];   // resize buffer to actual number of bytes

	/* read in the contents of CONVERTED */
	i = 0;
	tree = make_jrb();   // make the main tree
	while(i < length) {
		/* CREATE VAL STRUCT */
