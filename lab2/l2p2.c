 ...
 * This program reads information from the file converted and
   ...
   This program strictly uses system calls.
 ***************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
...
main(int argc, char **argv)
{
	...
	fileCopy = open("converted", O_RDONLY);  // fileCopy is used for counting bytes
	if (file==-1 || fileCopy==-1) { perror(argv[1]); exit(1); }

	/* get the number of bytes within file */
	bytes = 0;
	byteCount = 0;
	while(read(fileCopy,&c, 1) == 1){
		byteCount++;
		bytes++;
	}

	/* read actual file into buffer */
	unsigned char buffer[byteCount];  // resize buffer
	bytes = read(file, buffer, byteCount);
