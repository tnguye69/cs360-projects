 ...
 * cs360 - lab 7
 * jsh3.c 
 * Overview: Jsh3 is a very primitive c-interpreter shell that 
			 will let you execute commands and redirect their
             input/output. Jsh will allow you to use the 
             operations <, >, >>, &, and |. JSH3 implements
			 pipes.
*************************************************************/
#include <stdio.h>
#include <string.h>
			...
			if(is->NF==1){  // there's only 1 word command
				...
				for(i=0; i<is->NF; i++){
					...
					/* if | */
						...
						newargv[j] = NULL;  // we want to reset newargv everytime we see a pipe
						j = 0;
						if(pipe(pipefd) < 0){  // call pipe on the two processes
							perror("pipe error");
							exit(1);
						}
						...
						int id = fork();
						if(id == 0){
							if(readFile != -1){  // if we have file to read, redirect stdin to command
								if(dup2(readFile, 0) != 0){
									perror("dup2 error on readFile");
									exit(1);	
								}
								close(readFile);
								readFile = -1;
							...
							if(dup2(pipefd[1], 1) != 1){  // redirect the stdout to command or next pipe
								perror("error on pipefd[1]");
								exit(1);
							}	
							close(pipefd[0]);
							close(pipefd[1]);

							/* call execvp on the process once pipes have been set */
							execvp(newargv[0], newargv);
							perror(newargv[0]);
							exit(1);
						}
						else{
							if(readFile != -1) close(readFile);  // close file if one is open
							readFile = pipefd[0];  // redirect stdin to readFile
							close(pipefd[1]);  // close out pipefd[1]
						}
					}
					else{
